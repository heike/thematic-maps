...
Here we can finally start writing the paper. Figure~\ref{fig:map-agperc} shows a choropleth map of the percentage of agricultural by departments in France in 1954. 

\begin{figure}
\centering
<<old-france, echo=FALSE, out.width='.5\\linewidth'>>=
library(ggplot2)
library(ggthemes)
